...
1. **`bbox`** (toạ độ highlight): CHƯA thêm. Spike highlight Phase-2-sớm quyết → khả thi thì thêm field + chạy lại worker trên kho (rẻ vì kho ~5 file). Không khả thi → bỏ luôn, schema giữ `page`.
2. **`pageEnd`** (đơn vị vắt nhiều trang): hiện chỉ `page` bắt đầu. Cân khi thiết kế Viewer/search Phase 2 — nếu "nhảy tới + biết đơn vị dài tới đâu" cần thì thêm. Slide không bao giờ cần (1 trang). Để ngỏ, không thêm sớm (YAGNI).
